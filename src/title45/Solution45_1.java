 ...
 * 题目：圆圈最后剩下的数字
 * 
 * 题目描述：每年六一儿童节,牛客都会准备一些小礼物去看望孤儿院的小朋友,今年亦是如此。HF作为牛客的资深元老,
 * 自然也准备了一些小游戏。其中,有个游戏是这样的:首先,让小朋友们围成一个大圈。然后,他随机指定一个数m,让编号为0的小朋友开始报数。
...
public class Solution45_1 {
	public int LastRemaining_Solution(int n, int m){
        ...
        int index = (m-1) % circle.size();
        while(circle.size()> 1){
        	circle.remove(circle.get(index));
